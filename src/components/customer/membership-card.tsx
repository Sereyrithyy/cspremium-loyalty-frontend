  ...
  function MembershipCard({ customer, className }, ref) {
    const qrValue =
      typeof window !== "undefined"
        ? `${window.location.origin}/member/${customer.qrToken}`
        : `/member/${customer.qrToken}`;
          ...
          <div className="relative h-full w-full rounded-2xl bg-linear-to-br from-[#1e1a24] via-[#16131c] to-[#0d0b10] px-5 py-4">
            <div className="relative flex h-full flex-col justify-between">
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  <div className="relative mb-2">
                    <Image
                      ...
                      height={100}
                      priority
                      className="h-12 w-12 sm:h-21 sm:w-21 object-contain transition-opacity hover:opacity-80"
                    />
                  </div>
                  {/* <div>
                ...
                <div className={`rounded-full bg-linear-to-br ${tierColor} px-3 py-1 text-[9px] sm:text-[12px] font-semibold uppercase tracking-widest shadow-lg shadow-gold/5 ring-1 ring-gold/20 backdrop-blur-sm`}>
                  {customer.tier}
                </div>
              </div>
